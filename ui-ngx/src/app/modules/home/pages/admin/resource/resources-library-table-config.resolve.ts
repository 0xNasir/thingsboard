...
export class ResourcesLibraryTableConfigResolver implements Resolve<EntityTableConfig<Resource, PageLink, ResourceInfo>> {
  ...
  constructor(private store: Store<AppState>,
    ...
    this.config.columns.push(
      ...
      new EntityTableColumn<ResourceInfo>('title', 'resource.title', '40%'),
      new EntityTableColumn<ResourceInfo>('fileName', 'File name', '40%'),
      new EntityTableColumn<ResourceInfo>('resourceType', 'resource.resource-type', '20%',
    ...
    this.config.cellActionDescriptors.push({
      name: 'Copy absolute link',
      icon: 'copy_all',
      isEnabled: () => true,
      onAction: ($event, entity) => this.copyResource($event, entity, 'full')
    });
    this.config.cellActionDescriptors.push({
      name: 'Copy content link',
      icon: 'content_copy',
      isEnabled: () => true,
      onAction: ($event, entity) => this.copyResource($event, entity, 'content')
    });
  ...
  copyResource($event: Event, resource: ResourceInfo, cType: string) {
    if ($event) {
      $event.stopPropagation();
    }
    let link = (cType == 'full' ? location.protocol + '//' + location.hostname + ':' + location.port : '') + '/api/content/public/' + resource.fileName
    this._clipboardService.copy(link);
    this.store.dispatch(new ActionNotificationShow(
      {
        message: 'Resource link is copied to clipboard!',
        type: 'success',
        duration: 750,
