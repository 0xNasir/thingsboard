...
export class WidgetEditorComponent extends PageComponent implements OnInit, OnDestroy, HasDirtyFlag {
  ...
  themeName=localStorage.getItem('ace_theme')||'Monokai';
  themeList=['Textmate', 'Github', 'Ambiance', 'Monokai', 'Chaos', 'Dracula', 'Eclipse', 'Terminal', 'Tomorrow', 'Tomorrow_night', 'Twilight', 'xCode']
    ...
    return getAce().pipe(
      map((ace) => {
        ...
        if (localStorage.getItem('ace_theme')){
          aceEditor.setTheme('ace/theme/'+localStorage.getItem('ace_theme'));
        }else{
  ...
  changeTheme(theme: string) {
    localStorage.setItem('ace_theme', theme.toLowerCase());
    this.aceEditors.forEach(editor=>{
      editor.setTheme('ace/theme/'+theme.toLowerCase());
    });
    this.store.dispatch(new ActionNotificationShow(
      {message: 'Theme is changed to '+theme, type: 'success'}));
  }
}
