...
export class HomeComponent extends PageComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  logo = 'assets/Syncross-logo-white.svg';
  ...
  constructor(protected store: Store<AppState>,
    ...
    super(store);
    this.primaryToolbarBgColor=localStorage.getItem('satlPrimaryToolbar')||'#ff0000';
    this.drawerLogoBgColor=localStorage.getItem('satlDrawerLogoBgColor')||'#000000';
    this.leftSideMenuBgColor=localStorage.getItem('satlDrawerBgColor')||'#000000';


    this.theme=localStorage.getItem('satlTheme');
    if (this.theme=='light'){
      this.document.documentElement.classList.remove('tb-dark');
    }else{
      this.document.documentElement.classList.add(this.theme=='light'?'tb-default':'tb-dark');
    }
  }

  ngOnInit() {
    this.bottomLeftHtml = window['bottomLeftHtml'];
  ...
  changeTheme() {
    ...
    localStorage.setItem('satlTheme', this.theme);
    if (this.theme=='light'){
      this.document.documentElement.classList.remove('tb-dark');
    }else{
      this.document.documentElement.classList.add(this.theme=='light'?'tb-default':'tb-dark');
