...
export class RepositorySettingsComponent extends PageComponent implements OnInit {
  ...
  ngOnInit() {
    this.color = localStorage.getItem('satlTheme') == 'dark' ? '#FFFFFF' : this.color;
  ...
  checkAccess(): void {
    ...
    this.adminService.checkRepositoryAccess(settings).subscribe(() => {
      this.store.dispatch(new ActionNotificationShow({
        message: this.translate.instant('admin.check-repository-access-success'),
        type: 'success'
      }));
    });
  }
      ...
      if (data) {
        this.adminService.deleteRepositorySettings().subscribe(
          () => {
            ...
            this.repositorySettingsForm.reset({
              defaultBranch: 'main',
              authMethod: RepositoryAuthMethod.USERNAME_PASSWORD
            });
            this.updateValidators(false);
