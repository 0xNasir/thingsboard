...
export class AppComponent implements OnInit {
  ...
  savePrimaryColor() {
    this.primaryColorPalette = this.computeColors(localStorage.getItem('satlUIPrimaryColor') || '#2196f3');
    this.updateTheme(this.primaryColorPalette, 'primary');
    const accentColorPalette = this.computeColors(localStorage.getItem('satlUIAccentColor') || '#ffc107');
    this.updateTheme(accentColorPalette, 'accent');
    const warnColorPalette = this.computeColors(localStorage.getItem('satlUIwarnColor') || '#f44336');
    this.updateTheme(warnColorPalette, 'warn');
  ...
  updateTheme(colors: Color[], theme: string) {
    colors.forEach(color => {
      ...
      document.documentElement.style.setProperty(
        ...
        color.darkContrast ? '#000000' : '#ffffff'
      );
    });
  }
