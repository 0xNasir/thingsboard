...
export class PgRestService {
  ...
  getData(query: string):Observable<any>{
    return this.httpClient.get(environment.apiUrl+`/ap/api/pg/${query}`);
  }

  getCustomData(query: string, value: any):Observable<any>{
    return this.httpClient.post(environment.apiUrl+`/ap/api/pg/rpc/${query}`, value);
  }
